...
(use-package rust-mode
  ...
  (defun my/rust-format-and-organize ()
    "Format buffer and optionally organize imports for Rust."
    (interactive)
    (eglot-format-buffer)
    ;; rust-analyzer doesn't have organize-imports as a built-in code action
    ;; but we can still trigger other useful actions
    (when (fboundp 'eglot-code-actions)
      (eglot-code-actions nil nil "source.organizeImports")))

  :bind (:map rust-mode-map
              ("C-c C-c" . #'my/rust-format-and-organize)
              ("C-c C-t" . #'rust-test)))
