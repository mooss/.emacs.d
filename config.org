...
#+begin_src emacs-lisp
;; Add important paths to the exec-path that Emacs uses to locate binaries.
(mapc (lambda (d) (add-to-list 'exec-path (expand-file-name d))) '("~/.local/bin" "~/.rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin/"))
;; Also pass them to any child processes so they inherit the same PATH.
...
#+begin_src emacs-lisp
;; Better scrolling.
(setq scroll-step 2                ; Scroll 1 line at a time.
...
;; Supposed to install the required fonts locally if it does not work install `nerd-fonts` or a
;; subset like (`ttf-nerd-fonts-symbols(-mono)?`).
...
;; Conf from https://github.com/Wilfred/helpful.
...
Version 31.1 completely broke splitting (it only seems to care about monitor proportion and not window proportion).
This is barely a fix:
#+begin_src emacs-lisp
(defun my-split-window-by-shape (&optional window)
  "Split WINDOW based on its proportions, ignoring split thresholds.

If WINDOW is at least as tall as it is wide, split it vertically
(the new window appears below); otherwise split it horizontally
(the new window appears on the right).

If the preferred direction isn't physically possible, fall back to
the other direction.  Return the new window, or nil if WINDOW can't
be split at all.

Intended for use as `split-window-preferred-function'."
  (setq window (or window (selected-window)))
  (let ((split-below-p (>= (window-pixel-height window)
                           (window-pixel-width window)))
        ;; Ignore `split-height-threshold' / `split-width-threshold':
        ;; shape alone should decide the direction.
        (split-height-threshold 0)
        (split-width-threshold 0))
    (cond
     ((and split-below-p (window-splittable-p window nil))
      (split-window window nil 'below))
     ((and (not split-below-p) (window-splittable-p window t))
...
(setopt split-window-preferred-function #'my-split-window-by-shape)
#+end_src
...
(dolist (lang '(bash typescript css elisp go python))
...
(setq lsp-typescript-prefer-use-project-ts-server t)
#+end_src
...
Fix insufferable behavior where Rust files ask if I want to save *when I explicitely asked to save*:
#+begin_src emacs-lisp
(defun my-rustic-save-some-buffers (&optional pred)
  "Save all rustic buffers without asking."
  (save-some-buffers (or pred t) nil))

(advice-add 'rustic-save-some-buffers :override #'my-rustic-save-some-buffers)
#+end_src

*** Python

Install the language server with something like:
 - =pip install pyright=
 - =pacman -Sy pyright=

Enable eglot with =python-mode=:
#+begin_src emacs-lisp
(use-package python
  :ensure t
  ...
  :config
  (defun my/python-format-and-organize ()
    "Format buffer and organize imports for Python."
    (interactive)
    (eglot-format-buffer)
    (call-interactively #'eglot-code-action-organize-imports))
  :bind (:map python-mode-map ("C-c C-c" . my/python-format-and-organize)))

(use-package pyvenv
  :ensure t
  :config
  (defun my/pyvenv-restart-eglot ()
    "Restart eglot to pick up the new Python environment."
    (when (eglot-current-server)
      (call-interactively #'eglot-reconnect)))
  (add-hook 'pyvenv-post-activate-hooks #'my/pyvenv-restart-eglot)
  (add-hook 'pyvenv-post-deactivate-hooks #'my/pyvenv-restart-eglot))
#+end_src
...
(use-package corfu
  ...
  (corfu-auto nil)   ;; Automatically open the autocompletion menu.
...
Stop the whining of =so-long=:
#+begin_src emacs-lisp
(with-eval-after-load 'so-long
  (add-to-list 'so-long-mode-preserved-minor-modes 'magit-mode)
  (add-to-list 'so-long-target-modes 'magit-mode t))
#+end_src
...
American dates when inserting timestamps:
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(use-package ob-go)
(use-package ob-typescript) ; no syntactic coloration, need to somehow bind typescript-ts to it.

(let ((file "~/git/ob-racket/ob-racket.el"))
  (if (file-exists-p file)
      (load-file file)
    (message "Error: %s not found! Clone https://github.com/togakangaroo/ob-racket" file)))
#+end_src
...
(use-package aidermacs
  ...
  :config
  (setenv "OPENROUTER_API_KEY" (get-eonwe-api-key))
  :custom
  (aidermacs-default-model "openrouter/deepseek/deepseek-v4-flash-0731")
  (aidermacs-default-chat-mode 'architect)
  (aidermacs-extra-args '("--no-suggest-shell-commands")))
#+end_src

#+begin_src emacs-lisp
(setq aidermacs-architect-model "openrouter/deepseek/deepseek-v4-flash-0731")
;(setq aidermacs-editor-model "openrouter/openai/gpt-oss-120b")
(setq aidermacs-editor-model "openrouter/deepseek/deepseek-v4-flash-0731")
;(setq aidermacs-weak-model "openrouter/openai/gpt-oss-120b")
#+end_src
